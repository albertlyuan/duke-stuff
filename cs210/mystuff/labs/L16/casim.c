...
unsigned long get_tag (unsigned long addr, unsigned long num_sets, unsigned short bsize) {
    ...
    int setOffset = log2(num_sets);
    int blockOffset = log2(bsize);
    unsigned long mask = ~0;
    mask = mask << (setOffset+blockOffset);

    unsigned long ret = addr & mask;
    ret = ret >> (setOffset+blockOffset);
...
unsigned long get_index (unsigned long addr, unsigned long num_sets, unsigned short bsize) {
    ...
    mask = mask << (64-(setOffset+blockOffset));
    mask = mask >> (64-(setOffset+blockOffset));
    mask = mask >> blockOffset;
    mask = mask << blockOffset;

    unsigned long ret = mask & addr;
    ret = ret >> blockOffset;
