...
unsigned int get_vpn(unsigned int address) {
    // TODO: Edit this
    // return the virtual page number for the provided address
    int mask = ~0;
    int shift = 12;
    int ret;
...
unsigned int nextPageOutArray[VM_PAGES];
int validCount = 0;
int FIFOqueuePointer = 0;
void allocate_phys_page(pte_t *pagetable, unsigned int vpn) {
    // TODO: Edit this
    int FIFOqueue = FIFOqueuePointer % 64;
    unsigned int replaced = nextPageOutArray[FIFOqueue];

    pagetable[replaced].valid=0;
    if (verbose){
        if (FIFOqueuePointer >= 64){
            print_replacement(replaced, FIFOqueue);
        }
    }

    pagetable[vpn].phys_page = FIFOqueue;
    pagetable[vpn].valid=1;

    nextPageOutArray[FIFOqueue] = vpn;
    FIFOqueuePointer++;
    return;
