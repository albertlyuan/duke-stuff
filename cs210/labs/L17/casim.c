...
#else
    unsigned short block_bits = log2(bsize);
    unsigned short index_bits = log2(num_sets);
//    unsigned long tag = (addr & (unsigned long) ~(bsize-1));
    unsigned long tag = (addr >> (block_bits + index_bits));
      return tag;
#endif
...
#else
    unsigned short block_bits = log2(bsize);
    return (addr >> block_bits) % num_sets;
#endif
}
