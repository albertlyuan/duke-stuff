...
// return node at head of free list, allocate NUM_ALLOC more nodes using malloc if free list empty
node_t *get_node() {
#ifdef STUDENT
    return NULL;
#else
    ...
    if(!free_list) {  // empty free list, need to get more
        free_list = (node_t *) malloc(NUM_ALLOC * sizeof(node_t));
        memset(free_list, 0, NUM_ALLOC * sizeof(node_t));
...
// insert freed nodes at head of free list
