...
my $NUMPHASES = 4;   # number of phases (not counting secret phase)
my $quiet;
...
if ($notify) {
    ...
    # http://www.perlmonks.org/?node_id=233031
    my @chars = (a..z, A..Z, 0..9);
    $user_password = join '', map { @chars[rand @chars] } 1 .. 20;
}
else {
    $bombflags="";
    $notifyflag="";
...
#
system("(cd $srcdir; export CFLAGS='$Bomblab::CFLAGS'; export SERVERNAME=$Bomblab::SERVER_NAME; export SERVERPORT=$Bomblab::RESULTD_PORT; export USERID='$username'; export USERPWD='$user_password'; export LABID='$labid'; export BOMBFLAGS='$bombflags'; export NOTIFYFLAG='$notifyflag'; export BOMBPHASES='$phasearg'; export BOMBID=$bombid; make -e bomb; make -e bomb-solve; make solution.txt) $redirect ") == 0
...
#
# Create the PASSWORD file with a random string to authenticate web requests belonging to this student
#
if ($notify) {
    system("(echo $user_password > $bombdir/PASSWORD) $redirect") == 0
        or goodbye("Could not write to $bombdir/PASSWORD file");
}
